...
public class CrystalChoice : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(dec.CrystalPickedUp == true)
        {
            gameObject.GetComponent<CrystalChoice>().enabled = false;
        }
    }
    ...
    private void Update()
    {
        ...
        if (Input.GetButtonDown("Submit") && inZone == true)
        {
            ...
            FindObjectOfType<Decisions>().CrystalPickedUp = true;
        }
        ...
        if (cMan.choice == 2f && dMan.dialogueActive == false && whatChoice == 0f && inZone == true && Input.GetButtonUp("Submit"))
            ...
            FindObjectOfType<Decisions>().CrystalPickedUp = false;
        }
